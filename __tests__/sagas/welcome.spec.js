...
describe('welcome saga', () => {
  ...
  it('should handle an invalid token correctly', () => {
    jest.resetModules();
    const { TokenInvalidError } = jest.requireActual('../../app/utils/SERVER_URL');
    expectSaga(welcomeSaga)
      .provide([
        [select(tokenSelector), 'token'],
        [
          matchers.call.like({ fn: apiRequest, args: ['events'] }),
          throwError(new TokenInvalidError('responseCopy')),
        ],
      ])
      .dispatch(welcomeActions.refresh())
